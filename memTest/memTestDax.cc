...
#define DEFAULT_MEM_FILE "/dev/mem"
#else
#define FATAL do { fprintf(stdout, "Error at line %d, file %s (%d) [%s]\n", \
		__LINE__, __FILE__, errno, strerror(errno)); goto fatal_out; } while(0)
#define DEFAULT_MEM_FILE "/dev/dax0.0"
#endif
...
template<typename T, unsigned int N> bool
rwTest(void *addr, uint64_t *bytes)
{
	...
	if (gWR) {
		fprintf(stdout, "[WR] *(u%ld *)%p=0x%llx\n", sizeof(T) * 8, addr, (unsigned long long)wdat);
		*((volatile T*) addr) = wdat;
	}

	if (gRD) {
		fprintf(stdout, "[RD] rdat=*(u%ld *)%p\n", sizeof(T) * 8, addr);
...
int main(int argc, char **argv)
{
	...
	if (!physAddr) {
		...
		strcpy(devFile, DEFAULT_MEM_FILE);

		optind += 1;
	}

	fprintf(stdout, "devFile: %s\n", devFile);
	...
	offset_in_page = (unsigned)(physAddr & (uint64_t) (page_size - 1));
	...
	fprintf(stdout, "fd = %d\n", fd);
	fprintf(stdout, "map_base = %p\n", map_base);
	fprintf(stdout, "virt_addr = %p\n", virt_addr);
